...
import {Letter, User, UserBadge} from "@/types/object";
...
const UserFeed: React.FC<UserFeedProps> = ({user, letters = [], userBadge = []}) => {
  const router = useRouter();
  const currentUser = useUser();
  const letterEmpty = letters.length === 0;
  const badgeEmpty = userBadge.length === 0;
      ...
      <section className="w-full flex-col pt-[20px] pb-[30px] gap-y-[18px]">
        ...
        <article className="box w-full p-4 mt-[18px]">
          ...
          <div className="flex gap-x-3">
            {badgeEmpty && (<div className="flex flex-col pl-5 pt-[75px]">
              <Typo>아직 획득한 뱃지가 없습니다!</Typo>
              <Typo>편지를 보내고, 뱃지를 모아보세요 :)</Typo>
            </div>)}
          </div>
          <section className="flex gap-x-1.5">
            {userBadge.map((userBadge) => (
              <img src={userBadge.badge.icon} key={userBadge.id} alt={userBadge.badge.name}/>
            ))
            }
          </section>
        </article>
